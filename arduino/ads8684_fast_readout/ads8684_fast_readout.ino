...
struct {
  ...
  unsigned int chansWord = 0; // binary list of channels to turn on
} globals;
...
void printStatus(){
  ...
  */
  ...
  txDoc["stdev"] = stdev;
  txDoc["chansWord"] = globals.chansWord;
  txDoc["chanSeq"] = bank.getChannelSequence();
  for (i=0; i<8; i++) {
    txDoc["chans"][i] = ((globals.chansWord >> i) & 1);
  }
  serializeJson(txDoc, Serial);
  Serial.write('\n');
}
...
void chansCmd(){
  ...
  for (i=0; i<rxDoc["chans"].size(); i++) {
    if (rxDoc["chans"][i] == 1) globals.chansWord += 1 << i;
  }
  bank.setChannelSPD(globals.chansWord);
  bank.autoRst();
}
